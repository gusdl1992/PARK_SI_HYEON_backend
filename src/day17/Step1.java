...
public class Step1 {
    ...
    public static void main(String[] args) {
        ...
        try { // 만약에 해당 클래스를 못찾았을때 대처.
            ...
            System.out.println("[시스템오류] : JDBC 드라이버 클래스를 찾지 못햇어." + e);
        }

        try { // 만약에 해당 연동할 DB 서버의 정보가 잘못되거나 DB 서버쪽에 문제가 발생 했을때 대처
            ...
            System.out.println("[시스템오류] : MYSQL 서버와 연동이 실패 했습니다." + e);
            // java.sql.SQLException: Access denied for user 'root'@'localhost' (using password: YES)
            // 계정이나 패스워드 오류일 가능성이 높다.
        }

    } // m e
} // c  e
// 1. Class.forName() : 현재 프로젝트내 클래스/라이브러리 찾고 메모리로 로딩한다.
            // Class.forName("com.mysql.cj.jdbc.Driver");
// 1. MYSQL 회사에서 Driver 클래스를 찾기 : com 폴더내 mysql 폴더내 cj 폴더내 jdbc 폴더내 driver 존재
// 2. 해당 폴더내 driver 클래스를 찾는 과정에서 오류가 발생할수도 있다.
...
// DriverManager.getConnection("DB 서버주소/DB명" , "계정명" , "비밀번호") : 연동해서 연동된 구현체 반환
//Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/test5" , "root" , "1234");
