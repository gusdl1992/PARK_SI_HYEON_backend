...
public class MemberController {
    ...
    // + 로그인 했다는 증거. - 로그아웃을 하기 전까지. [ 식별 가능한 중복없는 데이터 : 회원번호 , 아이디 ]
    String loginSession = null; // null 이면 비로그인 null 대신에 아이디가 있으면 로그인한 상태
    ...
    public String userFind(MemberDto memberDto){
        System.out.println("[2] MemberController.userFind");
        String newName = MemberDao.getInstance().userFind(memberDto);
        System.out.println("[4] newName = " + newName);
    ...
    public String userPwFind(MemberDto memberDto){
        System.out.println("[2] MemberController.userPwFind");
        String newPw = MemberDao.getInstance().userPwFind(memberDto);
        System.out.println("[4] newPw = " + newPw);
